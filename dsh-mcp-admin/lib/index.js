...
export default {
  ...
  inject: ['webServer', 'loader', 'connection'],
  ...
  apply(ctx, config = {}) {
    ...
    const gate = (req, res) => {
      ...
      // ---------- 认证：必须显式做，且必须用官方那一套 ----------
      //
      // 下面挂路由的地方用 kind:'exact' 注册 /api/<method>。dsh 的认证不在路由框架里，
      // 而是挂在 client-connection 注册的 kind:'prefix' /api 路由的 handler 第一行
      // （`connection.requestRejection(req)`）。**exact 先于 prefix 匹配**，那条 prefix
      // 路由根本轮不到执行 —— 插件路由实际上是裸奔的。
      //
      // 实测后果（修复前）：不带任何 cookie 直接 POST /api/mcp.setEnabled 等端点会被
      // 正常受理。本插件能改 MCP 服务器配置，等同于在目标机器上增删可执行的外部进程。
      //
