...
`dsh-bridge-center`（插件中心主插件/管理桥）之上的 **MCP 子插件**：只负责 **MCP 服务器实例**（`mcp.*`）的增删改查/启停/重启。它是**普通插件，不受保护**——可以被插件中心正常**启用 / 停用 / 卸载**。
...
后端需先装插件中心主插件 `dsh-bridge-center`（App 一键安装/`dsh plugin add`）。
...
- 本包**不受保护**——能正常被插件中心操作；主插件 `dsh-bridge-center` 才受保护。
