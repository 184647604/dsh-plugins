...
/**
 * `center.restart` 从「决定重启」到真正 `process.exit(0)` 的等待。
 *
 * 必须留出这段时间：HTTP 响应还在往 socket 里写，立刻退出会让客户端收到连接被
 * 重置而不是 `{ok:true}`，用户看到的是「重启按钮点了没反应」。300ms 足够写完
 * 一个几百字节的响应，又不至于让人等得明显。
 */
...
export default {
  ...
  apply(ctx, config = {}) {
     ...
     * 判断当前进程是否被守护进程托管。
     *
     * 先看环境变量：各家守护进程都会在自己的子进程里留下痕迹，这比解析进程树
     * 可靠得多，也跨平台。都查不到再退回 POSIX 的「被 init 收养」（ppid===1）。
     *
     * Windows 上 ppid 没有 1 这个语义（父进程是 cmd.exe / explorer.exe），
     * 所以那里只能靠环境变量判断 —— 结论通常是「没有守护进程」，正是我们要的。
     */
     ...
     * （npm 生成 dsh.cmd 直接跑 `node lib/bin.js`），实测 PC 端
     ...
     *     进程抢同一个端口，反而更糟。
     *  2. 没有（手动终端 / Windows 启动文件夹 / tmux）→ 派一个 detached 的小助手，
     *     等本进程真正退出、端口释放之后，再用**同一个命令行**把 dsh web 拉起来。
     *     助手用 `node -e` 内联、经 spawn 数组传参（不经 shell，因此没有引号问题）。
     *
     ...
     *
     * payload:
     *   - `dryRun` (boolean)：只回报将要执行的动作，不真的做。客户端可据此在弹窗里
     *     展示「将用哪种方式重启」，也方便排查。
     */
    const opCenterRestart = async (payload) => {
      ...
      // 无守护进程：派助手等待本进程退出后再拉起。
      const helper = [
        "const {spawn}=require('node:child_process');",
        "const a=process.argv.slice(1);",
        "const exe=a[0],ppid=Number(a[1]),args=a.slice(2);",
        "const alive=(p)=>{try{process.kill(p,0);return true}catch{return false}};",
        "const t0=Date.now();",
        "const go=()=>{",
        "  if(alive(ppid)&&Date.now()-t0<30000)return setTimeout(go,200);",
        "  const c=spawn(exe,args,{detached:true,stdio:'ignore',env:process.env,windowsHide:true});",
      ...
      try {
        const child = spawn(
          process.execPath,
          ['-e', helper, process.execPath, String(process.pid), ...args],
          { detached: true, stdio: 'ignore', env: process.env, windowsHide: true }
        );
        child.unref();
      } catch (error) {
        // 助手没派出去就**不要**退出，否则 dsh web 直接没了。
        return {
          ok: false,
          error: {
      ...
      const timer = setTimeout(() => process.exit(0), RESTART_DELAY_MS);
      timer.unref?.();
      return { ok: true, value: { scheduled: true, dryRun: false, plan } };
    };
